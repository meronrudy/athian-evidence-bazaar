module Campaign
  class ObligationProfile < ApplicationRecord
    ...
    validates :external_id, :obligation_code, :name, :status, presence: true
    validates :external_id, uniqueness: true
    validates :status, inclusion: { in: Campaign::CountryProgram::READINESS_STATES }
    validates_campaign_metadata :required_evidence_json, :limitations_json

    private

    def assign_external_id
      self.external_id ||= "obl_#{SecureRandom.alphanumeric(20).downcase}"
    end
  end
end
